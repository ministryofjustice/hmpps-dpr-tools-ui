...
export function assertIsString(input: unknown, errorMessage: string): asserts input is string {
  if (typeof input !== 'string') {
    throw new AssertionError({ message: errorMessage })
  }
}

export function assertIsStringOrUndefined(input: unknown, errorMessage: string): asserts input is string | undefined {
  if (typeof input !== 'string' && input !== undefined) {
    throw new AssertionError({ message: errorMessage })
  }
}

export function assertIsAddressInfo(input: string | null | AddressInfo): asserts input is AddressInfo {
  if (input === null || typeof input === 'string') {
    throw new AssertionError({ message: 'Unexpected address type' })
  }
}

export function assertIsPlainObject(obj: unknown, errMessage: string): asserts obj is Record<string, unknown> {
  if (!isPlainObject(obj)) {
    throw new AssertionError({ message: errMessage })
  }
}

export async function pkceVerifierMatchesChallenge(verifier: string, challenge: CodeChallenge) {
...
export function assertIsValidTokenRequest(body: unknown): asserts body is TokenRequest {
