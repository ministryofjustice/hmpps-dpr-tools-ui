...
describe('helpers', () => {
  ...
  describe('assertIsPlainObject', () => {
    it.each(['nope', null, 1, false, []])('throws on wrong values (%s)', input => {
  ...
  describe('pkce', () => {
    describe('code_verifier', () => {
      it('should accept a valid PKCE code_verifier', () => {
        ...
        expect(isValidPkceCodeVerifier(verifier42.slice(0, verifier42.length - 1))).toBe(false)
      })
      ...
      it('should throw on an unsupported method', async () => {
        const verifier = createPKCEVerifier()
        await expect(createPKCECodeChallenge(verifier, 'BAD-METHOD' as PKCEAlgorithm)).rejects.toThrowError(
          'Unsupported PKCE method ("BAD-METHOD")',
        )
      })
    })
  })
