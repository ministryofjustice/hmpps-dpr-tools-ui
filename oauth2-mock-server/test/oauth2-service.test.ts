...
describe.each(['https://issuer.example.com', 'https://issuer.example.com/'])(
  ...
  (issuerUrl: string) => {
    ...
    it('should use custom endpoint paths', async () => {
      ...
      const res = await request(customService.requestHandler).get('/custom-well-known').expect(200)
      ...
      // GET
      await Promise.all(
        getTestCases.map(async ([path, expectedStatus, query]) => {
      ...
      await Promise.all(
        postTestCases.map(async ([path, expectedStatus]) => {
          await request(customService.requestHandler).post(path).expect(expectedStatus)
        }),
      )
    })
    ...
    it('should expose an OpenID configuration endpoint', async () => {
      const res = await request(service.requestHandler).get('/.well-known/openid-configuration').expect(200)
    ...
    it('should expose the userinfo endpoint', async () => {
      const res = await request(service.requestHandler).get('/userinfo').expect(200)
    ...
    it('should allow customizing the userinfo response through a beforeUserinfo event', async () => {
      ...
      const res = await request(service.requestHandler).get('/userinfo').expect(401)
    ...
    it('should expose CORS headers in an OPTIONS request', async () => {
      const res = await request(service.requestHandler).options('/token').expect(204)
    ...
    it('should expose a token introspection endpoint that returns information about a token', async () => {
      const res = await request(service.requestHandler).post('/introspect').type('form').expect(200)
    ...
    it('should allow customizing the introspect response through a beforeIntrospect event', async () => {
      ...
      const res = await request(service.requestHandler).post('/introspect').expect(200)
    ...
    describe('PKCE', () => {
      it('should grant access in normal PKCE flow with SHA-256 code_verifier', async () => {
        ...
        const resAuth = await request(service.requestHandler).get('/authorize').query(searchParams.toString())
...
function tokenRequest(app: RequestListener) {
  return request(app).post('/token').type('form').expect('Cache-Control', 'no-store').expect('Pragma', 'no-cache')
}
