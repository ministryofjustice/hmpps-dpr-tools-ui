...
describe('JWK Store', () => {
  describe('generate()', () => {
    it.each([
      ...
      'should be able to generate a new %s based key (alg = %s)',
      async (_kind: string, alg: string, expectedKty: string) => {
    ...
    it.each(['RS123', 'dunno'])('throws on unsupported algs (alg = %s)', async (alg: string) => {
      const store = new JWKStore()

      await expect(() => store.generate(alg)).rejects.toThrow(
        'Invalid or unsupported JWK "alg" (Algorithm) Parameter value',
      )
    })

    it.each(['Ed007', 'dunno'])('throws on unsupported crv for EdDSA alg (crv = %s)', async (crv: string) => {
      const store = new JWKStore()

      await expect(() => store.generate('EdDSA', { crv })).rejects.toThrow(
        'Invalid or unsupported crv option provided, supported values are Ed25519 and Ed448',
      )
    })

    it.each([['RS256', ['e', 'n', 'd', 'p', 'q', 'dp', 'dq', 'qi']]])(
      'should return the private key of a key (alg = %s)',
      async (alg: string, expectedProps: string[]) => {
        const store = new JWKStore()
  ...
  describe('toJSON()', () => {
    it.each([undefined, true, false])(
      ...
      async (shouldIncludePrivates?: boolean) => {
        const store = new JWKStore()
        await store.generate('RS256', { kid: 'key-one' })
        await store.generate('RS256', { kid: 'key-two' })
        await store.generate('RS256', { kid: 'key-three' })
