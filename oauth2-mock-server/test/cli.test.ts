...
describe('CLI', () => {
  ...
  it.each([['-unknown'], ['--unknown'], ['123'], [' ']])('should not allow unrecognized options', async arg => {
    const res = await executeCli(arg)

    expect(res).toEqual(errorResponse(`Unrecognized option '${arg}'.`))
  })

  it.each([['0.0.0.0'], ['::'], ['localhost'], ['127.0.0.1'], ['::1']])(
  ...
  it.each([['not-a-number'], ['-1'], ['65536']])("should not allow invalid port number '%s'", async port => {
